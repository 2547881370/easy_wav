...
import configparser
import utils
from img2video import check_and_convert_image_to_video
import datetime

input_media = utils.find_image_or_video('in')
...
if input_media and input_vocal:
    utils.update_config('config.ini', 'OPTIONS', 'video_file', input_media)
    utils.update_config('config.ini', 'OPTIONS', 'vocal_file', input_vocal)
...
temp_output = os.path.join(working_directory,'temp','111.mp4')
...
while True:
  ...
  if preview_settings:
    ...
    subprocess.call([f'{ffmpeg_path}', "-loglevel", "error", '-i', temp_input_video, '-ss', str(preview_start_time), '-to', str(preview_start_time+preview_length_seconds), '-c', 'copy', preview_video_path])
    subprocess.call([f'{ffmpeg_path}', "-loglevel", "error", '-i', temp_input_audio, '-ss', str(preview_start_time), '-to', str(preview_start_time+1), '-c', 'copy', preview_audio_path])
  ...
  if os.path.isfile(temp_output):
    ...
    output_filename_timestamped = f"{filename}_{timestamp}{file_type}"
    final_output_path = os.path.join('out', output_filename_timestamped)
    shutil.move(temp_output, final_output_path)
    print(f"{filename} 的视频已成功创建于: {final_output_path}")
    
    #end processing timer and format the time it took
    end_time = time.time()
    elapsed_time = end_time - start_time
    formatted_setup_time = format_time(elapsed_time)
    print(f"制作耗时: {formatted_setup_time}")
